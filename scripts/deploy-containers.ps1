...
NEXT_PUBLIC_DOMAIN=reportmate.example.edu
"@
...
function Deploy-ToAzure {
    ...
    if ($AutoSSO) {
        $UpdateArgs += @(
            ...
            "--set-env-vars", "NEXT_PUBLIC_DOMAIN=reportmate.example.edu"
        )
    ...
    if ($AppUrl) {
        ...
        if ($AutoSSO) {
            ...
            Write-Success "Users visiting https://reportmate.example.edu will be automatically redirected to login"
        }
