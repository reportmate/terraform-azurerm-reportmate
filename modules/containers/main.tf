...
resource "azurerm_container_app" "frontend_prod_main" {
  ...
  template {
    container {
      ...
      env {
        name  = "HOST"
        value = var.enable_custom_domain && var.custom_domain_name != "" ? var.custom_domain_name : "reportmate.example.edu"
      }

      env {
        name  = "NEXT_PUBLIC_SITE_URL"
        value = var.enable_custom_domain && var.custom_domain_name != "" ? "https://${var.custom_domain_name}" : "https://reportmate.example.edu"
      }
